...
import { Alert, ConfirmDeleteModal, Menu, MenuButton, MenuContent, Modal, Spinner } from 'components';

import { AxiosInstance } from 'axios';

import { putSourcesStationsService, postSourcesStationsService, deleteSourcesStationsService } from '@services';
...
import { Errors, SourcesFormatData, SourcesServerData, SourcesStationsData, StationData, ErrorResponse } from '@types';
...
const StationSourcesModel = ({
    ...
    const postSourcesStation = async () => {
        try {
            setLoading(true);
            const server = sourcesServers?.find((s) => `${s.fqdn} ${s.protocol} ${s.path} ${s.format}` === formState.server_id);
            ...
            const res = await postSourcesStationsService<ErrorResponse>(api, params);
    ...
    const putSourcesStation = async () => {
        try {
            ...
            const res = await putSourcesStationsService<ErrorResponse>(api, sourceStation?.api_id as number, params);
            if (res.statusCode >= 200 && res.statusCode < 300) {
                setMsg({
                    status: res.statusCode,
    ...
    const handleRemove = async () => {
        try {
            setLoading(true);
            const res = await deleteSourcesStationsService<ErrorResponse>(api, sourceStation?.api_id as number);
            if (res.statusCode >= 200 && res.statusCode < 300) {
                setMsg({
                    status: res.statusCode,
    ...
    useEffect(() => {
        if (sourceStation && type === 'edit') {
            const stringifiedSourceStation = Object.fromEntries(
                Object.entries(sourceStation).map(([key, value]) => [key, value?.toString()])
            );
            dispatch({
                type: 'set',
                payload: stringifiedSourceStation,
            });
            const server = sourcesServers?.find((sv) => sourceStation.server_id === sv.server_id);
            dispatch({
                type: 'change_value',
                payload: {
                    inputName: 'server_id',
                    inputValue: server ? matchServer(server.server_id) : '',
                },
            });
        }
    }, [sourceStation]);
                        ...
                        <div key={camp} className="w-full">
                            ...
                            >
                                ...
                                {['server_id', 'format'].includes(camp) && (
                                    <MenuButton setShowMenu={setShowMenu} showMenu={showMenu} typeKey={camp} />
                                )}
                            </label>
                            {showMenu?.show && showMenu?.type === 'server_id' && camp === 'server_id' && (
                                <Menu>
                                    {sourcesServers
                                        ?.filter((server) => {
                                            ...
                                            return searchWords.every((word) => serverInfo.includes(word.toLowerCase()));
                                        })
                                        .map((server) => (
                                            <MenuContent
