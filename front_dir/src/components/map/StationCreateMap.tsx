...
const StationCreateMap = ({
    ...
    const calculateDistance = (lat1: number, lon1: number, lat2: number, lon2: number): number => {
        ...
        const h = Math.sin((lat2Rad - lat1Rad) / 2) ** 2 + 
    ...
    const isWithinDistance = (lat1: number, lon1: number, lat2: number, lon2: number, maxDistance: number): boolean => {
    ...
    const stationsInRange = stations
        .filter(station => {
            if (!station.lat || !station.lon) return false;
            return isWithinDistance(currentMarker.lat, currentMarker.lng, station.lat, station.lon, rangeValue);
        })
        .map((s, index) => {
