...
class Cnn(pg.DB):
    ...
    def __init__(self, configfile, use_float=False, write_cfg_file=False):
        ...
        try:
            ...
            if write_cfg_file:
                create_empty_cfg()
                print(
                    ' >> No gnss_data.cfg file found, '
                    'an empty one has been created.'
                    'Replace all the necessary config and try again.')
                exit(1)
            else:
                raise
    ...
    def query(self, command, *args):
        ...
        for i in range(3):
            ...
            raise Exception(
                ('dbConnection.query failed after 3 retries. '
                    'Last error was: ') + str(err))
    ...
    def query_float(self, command, as_dict=False):
        ...
        for i in range(3):
            ...
            raise Exception(
                ('dbConnection.query_float failed after 3 retries. '
                    'Last error was: ')
                + str(err))
