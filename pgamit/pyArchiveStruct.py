...
This class handles the interface between the directory structure of the rinex
archive and the databased records. It can be used to retrieve a rinex path
based on a rinex database record. It can also scan the dirs of a supplied path
for d.Z and station.info files (the directories and files have to match the
...
class RinexStruct(object):
    ...
    def __init__(self, cnn):
        ...
        self.levels = cnn.query('SELECT rinex_tank_struct.*, keys.* FROM '
                                'rinex_tank_struct '
                                'LEFT JOIN keys ON keys."KeyCode" = '
    ...
    def insert_rinex(self, record=None, rinexobj=None):
        """
        Insert a RINEX record and file into the database and archive.
        ...
        if not self.get_rinex_record(NetworkCode=record['NetworkCode'],
            ...
            # check if we need to perform any rinex operations. We might be
            # inserting a new record, but it may just be a ScanRinex op where
            # we don't copy the file into the archive
            if rinexobj is not None:
            ...
            try:
                ...
                if rinexobj is not None:
                    ...
                    # copy fixed version into the archive (in case another
                    # session exists for RINEX v2)
                    ...
                    if rnx != rinexobj.rinex:
                        # update the table with the filename
                        # (always force with step)
                    ...
                    event = pyEvents.Event(
                        Description=(('A new RINEX was '
                                      'added to the archive: %s'))
                        % record['Filename'],
                    ...
                    event = pyEvents.Event(
                        Description=('Archived CRINEX file %s '
                ...
                if rinexobj and copy_succeeded:
                    # transaction rolled back due to error. If file made into
                    # the archive, delete it.
    ...
    def remove_rinex(self, record, move_to_dir=None):
        ...
        try:
            ...
            # check if this rinex file is the file that was processed and used
            # for solutions
            where_station = ('"NetworkCode" = \'%s\' '
                             'AND "StationCode" = \'%s\'') % (
                                record['NetworkCode'], record['StationCode'])
            rs = self.cnn.query(
                    ('SELECT * FROM rinex_proc WHERE %s '
                     'AND "ObservationYear" = %i AND "ObservationDOY" = %i')
                    % (where_station,
                       record['ObservationYear'],
                       record['ObservationDOY']))
            ...
            if rs.ntuples() > 0:
                self.cnn.query(
                    ('DELETE FROM gamit_soln WHERE %s '
                     'AND "Year" = %i AND "DOY" = %i')
                    % (where_station,
                       record['ObservationYear'], record['ObservationDOY']))

                self.cnn.query(
                    ('DELETE FROM ppp_soln WHERE %s '
                     'AND "Year" = %i AND "DOY" = %i')
                    % (where_station,
                       record['ObservationYear'], record['ObservationDOY']))

            # get the filename
            rinex_path = self.build_rinex_path(record['NetworkCode'],
                                               record['StationCode'],
                                               record['ObservationYear'],
                                               record['ObservationDOY'],
            ...
            self.cnn.query(
                ('DELETE FROM rinex WHERE %s AND "ObservationYear" = %i AND '
            ...
            if os.path.isfile(rinex_path):
                if move_to_dir:
                    ...
                    filename = Utils.move(rinex_path,
                                          os.path.join(
                                            move_to_dir,
                                            os.path.basename(rinex_path)))
                    description = ('RINEX %s was removed from the database '
                                   'and archive. '
                                   'File moved to %s. See next events '
                                   'for reason.') % (record['Filename'],
                                                     filename)
                else:
                    ...
                    description = ('RINEX %s was removed from the database '
                                   'and archive. '
                                   'File was deleted. See next events '
                                   'for reason.') % (record['Filename'])

            else:
                description = 'RINEX %s was removed from the database and '\
                              'archive. File was NOT found in the archive ' \
                              ('so no deletion was performed. See next events '
                               'for reason.') % (record['Filename'])
    ...
    def get_rinex_record(self, **kwargs):
        """
        Retrieve a single or multiple records from the rinex table given a set
        parameters. If parameters are left empty, it wil return all records
        matching the specified criteria. Each parameter acts like a filter,
        narrowing down the records returned by the function. The default
        behavior is to use tables rinex or rinex_proc depending on the
        provided parameters. E.g. if Interval, Completion and Filename are all
        left blank, the function will return the records using rinex_proc.
        Otherwise, the rinex table will be used.
        ...
        if any(param in ('Interval', 'Completion',
                         'Filename') for param in kwargs.keys()):
            table = 'rinex'
        else:
            table = 'rinex_proc'
    ...
    def build_rinex_path(self, NetworkCode, StationCode, ObservationYear,
                         ObservationDOY, with_filename=True, filename=None,
                         rinexobj=None):
        """
        Function to get the location in the archive of a rinex file. It has
        two modes of operation:
        1) retrieve an existing rinex file, either specific or the rinex for
            processing (most complete, largest interval) or a specific rinex
            file (already existing in the rinex table).
        2) To get the location of a potential file (probably used for
            injecting a new file in the archive. No this mode, filename has
            no effect.
        ...
        if not rinexobj:
            ...
            if filename:
                ...
                # if filename is set, user requesting a specific file:
                # query rinex table
                rs = self.cnn.query('SELECT ' + sql_string +
                ...
                rs = self.cnn.query(
                    ...
                    '\' AND "StationCode" = \'' + StationCode +
                    '\' AND "ObservationYear" = ' + str(
                        ObservationYear) + ' AND "ObservationDOY" = ' +
                    str(ObservationDOY))
            ...
            path = "/".join(
                str(field[level['rinex_col_in']]).zfill(level['TotalChars'])
                for level in self.levels)

            if with_filename:
                rnx_name = RinexNameFormat(field['Filename'])
                # database stores rinex, we want crinez
                return path + "/" + rnx_name.to_rinex_format(
                    pyRinexName.TYPE_CRINEZ)
            else:
                return path
            ...
            for level in self.levels:
                ...
                if len(kk) != level['TotalChars']:
                    raise ValueError('Invalid record \'%s\' for key \'%s\'' %
                                     (kk, level['KeyCode']))
    ...
    def parse_archive_keys(self, path_filename, key_filter=()):
        """
        based on a path and filename, this function parses the data and
        organizes the information in a dictionary
        key_filter allows to select which keys you want to get a hold on.
        The order of the keys in the path is given by the database table
        rinex_tank_struct
