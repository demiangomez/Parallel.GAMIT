...
def initialize(remote_pwd, params, solution_pwd):
...
def start(self):
    ...
    with open(os.path.join(self.pwd, 'monitor.log'), 'a') as monitor:
            ...
            try:
                with pyRinex.ReadRinex(rinex['NetworkCode'],
                    ...
                    if rinex['jump'] is not None:
                        monitor.write('                    -> RINEX file has been windowed: ETM detected jump on ' +
        ...
        create_replace_links(self)
        create_run_script(self)
        create_finish_script(self)

    # run the script to replace the links of the tables directory
    p = subprocess.Popen('find ./tables ! -name "otl.grid" -type l -exec ./replace_links.sh {} +', shell=True, stdout=subprocess.PIPE,stderr=subprocess.PIPE, cwd=self.pwd)
    _, _ = p.communicate()

    # now execute the run script
    p = subprocess.Popen('./run.sh', shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=self.pwd)

    stdout, stderr = p.communicate()

    p = subprocess.Popen('./finish.sh', shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=self.pwd)

    stdout, stderr = p.communicate()

    # check for any fatals
    p = subprocess.Popen('grep -q \'FATAL\' monitor.log', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=self.pwd)
    _, _ = p.communicate()
    ...
    return {'Session': '%s %s' % (self.params['NetName'], self.date.yyyyddd()), 'Success': success, 'NRMS': nrms, 'WL': wl, 'NL': nl, 'Missing': missing_sites}
...
def create_run_script(self):
    ...
    grep -q    "FATAL.*MAKEX/openf: Error opening file:.*batch" $OUT_FILE;
    if [ $? -eq 0 ]; then
        grep       "FATAL.*MAKEX/openf: Error opening file:.*batch" $OUT_FILE;
        echo "run.sh (`date +"%Y-%m-%d %T"`): MAKEX/openf: Error opening batch file" >> monitor.log
        echo "run.sh (`date +"%Y-%m-%d %T"`): will try sh_gamit again ..." >> monitor.log
        EXE=1;
    fi
