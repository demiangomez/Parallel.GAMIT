...
def rinex_task(ArchiveFile, NetworkCode, StationCode, StationAlias, date, stninfo, Window):

    # local directory as destiny for the CRINEZ files
    pwd_rinex = '/media/leleiona/aws-files'
...
def get_metadata(cnn, NetworkCode, StationCode, date, ObservationFYear):
    ...
    try:
        ...
        return None, None, None, None, None, traceback.format_exc() + ' processing ' + NetworkCode + '.' + StationCode + ' using node ' + platform.node() + '\n'
    ...
    if apr_tbl.ntuples() != 0:
        ...
        return None, None, None, None, None, '%s.%s has no PPP solutions and no APRs from last global run for %8.3f!' % (NetworkCode, StationCode, date.fyear)
    ...
    try:
        ...
        return None, None, None, None, None, '%s.%s has no metadata available for this date, but a RINEX exists!' % (NetworkCode, StationCode)
    ...
    aws_sync = cnn.query('SELECT * FROM aws_sync WHERE "NetworkCode" = \'%s\' AND "StationCode" = \'%s\' '
                         'AND "Year" = %i AND "DOY" = %i' % (NetworkCode, StationCode, date.year, date.doy))

    # everything ok, return information
    APR = '%s.%s %s %12.3f %12.3f %12.3f %5.3f %5.3f %5.3f %5.3f %5.3f %5.3f %s' % (
            NetworkCode, StationCode, StationAlias,
            Apr[0, 0], Apr[1, 0], Apr[2, 0], sigmas_xyz[0, 0], sigmas_xyz[1, 0], sigmas_xyz[2, 0],
            sigmas[1, 0], sigmas[0, 0], sigmas[2, 0], source.replace(' ', '_'))

    stninfo_line = stninfo.return_stninfo(stninfo.currentrecord).replace(StationCode.upper(), StationAlias.upper())

    if Window is not None:
        source += ' windowed from/to ' + Window.datetime().strftime('%Y-%M-%d %H:%M:%S')

    message = '%s.%s %s source of APR: %s' % (NetworkCode, StationCode, StationAlias, source)

    return StationAlias, APR, stninfo_line, stninfo.currentrecord, Window, message
...
def pull_rinex(cnn, date, Config, JobServer):
    ...
    rs = cnn.query('SELECT rinex_proc.* FROM rinex_proc '
                   'WHERE "ObservationYear" = %i AND "ObservationDOY" = %i' % (date.year, date.doy))
    ...
    for rnx in rinex:
        ...
        StationAlias, APR, stninfo_line, stninfo_rec, Window, message = get_metadata(cnn, rnx['NetworkCode'], rnx['StationCode'], date, rnx['ObservationFYear'])
        ...
        if stninfo_line is not None:
            with open('./' + metafile + '.info', mode='a') as fid:
                fid.write(stninfo_line + '\n')
        ...
        if APR is not None and stninfo_line is not None:
            ...
            if Config.run_parallel:
                JobServer.SubmitJob(rinex_task, (ArchiveFile, rnx['NetworkCode'], rnx['StationCode'], StationAlias, date, stninfo_rec, Window),
                                    (window_rinex, sigmas_neu2xyz),
                                    ('pyDate', 'pyRinex', 'os', 'numpy',
                ...
                callback[0].process_callback(rinex_task(ArchiveFile, rnx['NetworkCode'], rnx['StationCode'], StationAlias, date, stninfo_rec, Window))
                callback = output_handle(callback, metafile)
