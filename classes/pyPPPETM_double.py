...
class ETM():
    ...
    def __init__(self, cnn, NetworkCode, StationCode, plotit=False):
        ...
        if len(ppp) > 10:
            ...
            # careful! this SQL expression does not account for earthquakes happening the same day at different locations
            # it just finds the largest of the day
            jumps = cnn.query('select * from earthquakes right join '
                              '  (select "date" as fecha, max(mag) as magn from '
                              '  (select date("date"),mag from earthquakes) as eq group by date order by "date") as eqf on '
                              'date(earthquakes.date) = eqf.fecha and earthquakes.mag  = eqf.magn')
            ...
            self.jump_table = [[pyDate.Date(year=eq[0][0], month=eq[0][1], day=eq[0][2]).fyear, 1-numpy.exp(numpy.divide(-( numpy.square(eq[1]) + 5000),2*numpy.square(500)))] for eq in zip(eq[m > 0,3:6], dist[m > 0])]
            ...
            ts = numpy.arange(pyDate.Date(fyear=t.min()).mjd, pyDate.Date(fyear=t.max()).mjd+1, 0.1)
            ts = numpy.array([pyDate.Date(mjd=tts).fyear for tts in ts])
            ts = ts[:,numpy.newaxis]

            A, As, p_jumps, const_h, const_s = self.design_matrix(t, ts, np_jumps)
    ...
    def adjust_lsq(self, Ai, Pi, Li, constrains_h, constrains_s):
        ...
        if sc > 0:
            ...
            P[sp:sp+sc,sp:sp+sc] = 1e3*numpy.diag(numpy.ones(sc))
    ...
    def design_matrix(self, ti, ts, jumps):
        ...
        if ti.size > 1:
            ...
            Tr = numpy.min(ti)
            # offset
            ci = numpy.ones((ti.size,1))
            cs = numpy.ones((ts.size,1))
            ...
            # periodic terms
            si = numpy.array([sin(2 * pi * ti[:,0]), sin(4 * pi * ti[:,0]), cos(2 * pi * ti[:,0]), cos(4 * pi * ti[:,0])]).transpose()
            ss = numpy.array([sin(2 * pi * ts[:, 0]), sin(4 * pi * ts[:, 0]), cos(2 * pi * ts[:, 0]), cos(4 * pi * ts[:, 0])]).transpose()

            if jumps.size > 0:
                # remove jumps before and after the start of the data if they are just co-seismic of antenna changes (with no data)
                jumps = numpy.delete(jumps, numpy.where((jumps[:, 1] == 0) & ((jumps[:, 0] < ti.min()) | (jumps[:, 0] > ti.max()))), axis=0)
                # remove log decays that happened after max(t)
                jumps = numpy.delete(jumps, numpy.where((jumps[:, 1] != 0) & (jumps[:, 0] > ti.max())), axis=0)
                ...
                jlog_b4_start = jumps[(jumps[:, 1] != 0) & (jumps[:, 0] < ti.min())]
                ...
                hti = numpy.zeros((ti.shape[0], jumps.shape[0]))
                hts = numpy.zeros((ts.shape[0], jumps.shape[0]))

                for i,jump in enumerate(jumps):
                    # the jumps has to be within the t range to be applied
                    if jump[0] > ti.min() and jump[0] < ti.max():
                        hti[ti[:,0] > jump[0],i] = 1
                        hts[ts[:, 0] > jump[0], i] = 1
                        ...
                        if i > 0:
                            for j in range(i):
                                ...
                                if numpy.sum(numpy.logical_xor(hti[: ,j], hti[:, i])) < 3:
                ...
                hli = numpy.zeros((ti.shape[0], jumps[jumps[:, 1] != 0].shape[0] * 2))
                hls = numpy.zeros((ts.shape[0], jumps[jumps[:, 1] != 0].shape[0] * 2))
                n = jumps[jumps[:, 1] != 0].shape[0]

                for i,jump in enumerate(jumps[jumps[:, 1] != 0]):

                    if jump[1] != 0:

                        # determine if there is data to constrain the transients
                        min_t = numpy.min(ti[ti[:,0] > jump[0],0])

                        decay = 0.5

                        if min_t - jump[0] <= 0.02:
                            hli[ti[:, 0] >= jump[0], i + n] = numpy.log10(1 + (ti[ti >= jump[0]] - jump[0]) / 0.02)
                            hls[ts[:, 0] >= jump[0], i + n] = numpy.log10(1 + (ts[ts >= jump[0]] - jump[0]) / 0.02)

                        if min_t - jump[0] > 0.5:
                            decay = (min_t - jump[0])*2

                        hli[ti[:, 0] >= jump[0], i] = numpy.log10(1 + (ti[ti >= jump[0]] - jump[0]) / decay)
                        hls[ts[:, 0] >= jump[0], i] = numpy.log10(1 + (ts[ts >= jump[0]] - jump[0]) / decay)


                        #elif min_t - jump[0] < decay1 and min_t - jump[0] > decay2:

                        #    decay2 = min_t - jump[0]

                        #    hl1[t[:, 0] >= jump[0], i] = numpy.log10(1 + (t[t >= jump[0]] - jump[0]) / decay1)
                        #    hl1[t[:, 0] >= jump[0], i+n] = numpy.log10(1 + (t[t >= jump[0]] - jump[0]) / decay2)

                        #elif min_t - jump[0] < decay2:

                        #hl[t[:, 0] > jump[0], i] = numpy.log10(1 + (t[t > jump[0]] - jump[0]) / decay1)
                        #    hl1[t[:, 0] >= jump[0], i+n] = numpy.log10(1 + (t[t >= jump[0]] - jump[0]) / decay2)
                        ...
                        #        if numpy.sum(hl[:,j]) == 0:
                                    ...
                                    # no data to constrain the log decay
                ...
                hti = hti[:, (hti != 0).sum(axis=0) > 0]
                hts = hts[:, (hts != 0).sum(axis=0) > 0]
                hli = hli[:, (hli != 0).sum(axis=0) > 0]
                hls = hls[:, (hls != 0).sum(axis=0) > 0]

                jumps = jumps[jumps[:,0] != 0,:]

                Ai = numpy.column_stack((ci, vi, hti, hli, si))
                As = numpy.column_stack((cs, vs, hts, hls, ss))
                ...
                iht = 0
                for i, jump in enumerate(jumps):
                    if jump[1] != 0 and jump[0] > ti.min():
                        # check the data span
                        # take column i of ht
                        tspan = ti[numpy.where(hti[:,iht] == 1)]

                        #if tspan[0] - jump[0] > numpy.divide(float(5),float(365)):
                            # there is a data gap larger than 5 days after the jump. Constrain in
                        constrain = numpy.column_stack((numpy.zeros((1, 2 + iht)), 1, numpy.zeros((1, hti.shape[1] - iht - 1 + hli.shape[1])), numpy.zeros((1, 4))))
                ...
                Ai = numpy.column_stack((ci, vi, si))
                As = numpy.column_stack((cs, vs, ss))



            return Ai, As, jumps, constrains_h, constrains_s
